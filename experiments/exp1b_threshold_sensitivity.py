...
chosen percentile, whereas tuning-free adaptive rules (MAD/Otsu/calibrated-FPR)
auto-select a near-optimal operating point. Static single-node setting.
"""
import numpy as np
import pandas as pd
...
def run():
    ...
    fig, axes = C.plt.subplots(1, 2, figsize=(12, 4.6))
    for ax, ds in zip(axes, DATASETS):
        ...
        for name, _, _ in ADAPT:
            ax.axhline(np.mean(ad[name]["f1"]), ls="--", lw=2, color=ACOL[name],
                       alpha=0.9, label=f"{name} (tuning-free)")
    ...
    fig.suptitle("E1 motivation: F1 sensitivity to the manual threshold percentile "
                 "vs. tuning-free adaptive rules (mean$\\pm$std, 5 seeds)", y=1.03)
    C.savefig(fig, "exp1b_threshold_sensitivity")
    tab = pd.DataFrame(table)
    C.write_table(tab, "exp1b_threshold_sensitivity",
                  "Standalone-detector F1 under manual percentile selection vs. tuning-free "
                  "adaptive thresholds. The manual percentile must be searched; adaptive rules "
                  "match the best manual choice without tuning.", "tab:thr_sens")
